...
ADD CONSTRAINT check_cabinets_korpus
CHECK (korpus > 0);

ALTER TABLE "cabinets"
ADD CONSTRAINT check_cabinets_etazh
CHECK (etazh >= 0 AND etazh <= 30);

ALTER TABLE "sotrudniki"
ADD CONSTRAINT check_sotrudniki_phone
CHECK (phone ~ '^\+?[0-9\s\-\(\)]{5,20}$');

ALTER TABLE "oborudovanie"
ADD CONSTRAINT check_oborudovanie_year
CHECK (god_vipuska >= 2007 AND god_vipuska <= EXTRACT(YEAR FROM CURRENT_DATE));

ALTER TABLE "spisannoe_oborudovanie"
ADD CONSTRAINT check_spisannoe_year
CHECK (god_vipuska >= 2007 AND god_vipuska <= EXTRACT(YEAR FROM CURRENT_DATE));
